...
class Question(models.Model):
    ...
    image = models.ImageField(
        upload_to="questions_image", blank=True, null=True
    )
    ...
    def decrease_rank(self):    # f object race condition
        self.rank -= 1
        self.save()
...
class AnswerVote(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    voted_parent = models.ForeignKey(
        QuestionAnswer, null=True, blank=True, on_delete=models.SET_NULL
    )
    vote_value = models.CharField(
        max_length=10, null=True, blank=True, default='draft'
    )
